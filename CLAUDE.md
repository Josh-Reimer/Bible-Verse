...
- **`VerseLookUpActivity`** — receives a verse reference via `Intent` extra `"verse_ref"` (`"bookIndex:chapter:verse"`), splits it directly to extract book/chapter/verse integers without constructing a `Verse` object, then calls `getChapter()` once. The loop partitions lines into before/target/after `TextView`s by parsing the verse number from each raw asset line; lines with fewer than two colon-separated parts (e.g. the `"io exception"` error sentinel from `getChapter`) are skipped. Total: one file read per activity launch.
