...
### Core classes
- **`Bible`** — stateless; all methods take `Context`+`Tools` to load assets. `getChapter()` returns all verses in a chapter as a newline-joined string and breaks early once it passes the target chapter; `getVerse()` calls `getChapter()` and indexes into the split array; `getBookLength()` finds the chapter number on the last line to count chapters.
