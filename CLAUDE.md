...
- **`SimilarVerses`** — precomputed nearest-neighbor lookup for the "similar verses" feature. Lazily loads `assets/similar_verses.bin` into a single static `HashMap<String, String[]>` (cached across instances, mirroring `RedLetter`) and returns each verse's neighbor references. The whole table is generated offline (see `scripts/` below), so `getSimilar()` is a plain map hit — no model, no math, no NPU at runtime. The `.bin` is a compact big-endian format read via `DataInputStream`: a `uint32` count and a `uint8` K (neighbors per verse, currently 10), then per verse a `book,chapter,verse` triple (one unsigned byte each — every value is <256) followed by K neighbor triples, with `(0,0,0)` as a "no neighbor" sentinel (chapters/verses are 1-based so it never collides). K comes from the header rather than a constant, so regenerating with a different K needs no Java change. Parsing happens off the main thread (the actions sheet loads on a worker); references are translation-independent (KJV verse numbering is canonical) while the displayed text is rendered in the user's current translation.
