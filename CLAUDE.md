...
```
chapter:verse_number text of the verse
```
`Bible.java` reads these files via `Tools.getFile()` (which uses `AssetManager`) and parses them line-by-line with `BufferedReader`/`StringReader`.
...
### Core classes
- **`Bible`** — stateless; all methods take `Context`+`Tools` to load assets. `getChapter()` returns all verses in a chapter as a newline-joined string; `getVerse()` calls `getChapter()` and indexes into the split array; `getBookLength()` finds the chapter number on the last line to count chapters.
...
### Persistence
- **Room DB** (`bookmark_database`, `bookmark_dao`, `bookmark` entity) — table `bookmarks` with unique index on `bible_reference`. The DB is opened with `allowMainThreadQueries()`.
