...
- **`BrowseActivity`** — reached from the drawer ("Browse", above Settings and Bookmarks). Two stages in one Activity sharing one RecyclerView, which swaps adapter and layout manager between them: the 66 books in a `LinearLayoutManager` under OLD/NEW TESTAMENT headers (`BrowseBooksAdapter`, headers being `Bible.books` split at index 39, Matthew), then that book's chapters as a number grid (`BrowseChaptersAdapter`, `GridLayoutManager` with five columns on a phone and more as the screen widens). Picking a chapter opens `VerseLookUpActivity` with `"verse_ref"` = `book:chapter:0` — verse **0** on purpose, since it matches no verse, so the chapter opens from the top with nothing highlighted; browsing to a chapter is not arriving at a verse. Back and the toolbar's up arrow step grid → list → out, via an `OnBackPressedCallback` enabled only while the grid shows.

  Book names come from `Translations.properBook`, and `onResume` reloads them when the translation changed under Settings. The filter box matches the localised name *and* the English one (`Bible.getProperName`), so `rom` still finds Romans for a reader on the Spanish text — the same reasoning that has `VerseReferenceParser` register every translation's names at once. Both sides are folded through `VerseReferenceParser.foldAccents` (package-private for this) plus per-character `Character.toLowerCase`, never `String.toLowerCase()`, for the Turkish-locale reason the text-search path documents. A header with no books under it is dropped rather than left standing over an empty testament.

  `getBookLength()` reads a whole book file, so the chapter count behind the grid is fetched on a private executor (results cached per book, cleared on a translation change) — the book list stays up meanwhile rather than flashing an empty grid. Titles are set with `setTitle()`, not `toolbar.setTitle()`: AppCompat pushes the Activity title (the manifest `android:label`) onto the toolbar after `onCreate`, which silently undid a toolbar-only title on the restore path below.

  The last book/chapter opened lives in `SharedPreferences("settings")` under `"browse_last_book"`/`"browse_last_chapter"`, and a cold start reopens on that book's grid with the chapter outlined (`browse_chapter_tile_current`); up/back still steps out to the full list.
...
`SettingsActivity` opens a `MaterialTimePicker` from its own "Notification Time" row, shaped like every other row on that screen: a 20sp title on the left and the value on the right in `time_value_background` — the spinners' `spinner_box` minus the dropdown arrow (this opens a picker, not a menu) wrapped in a ripple. It was first built as the notification row's *summary line* carrying the time, and that failed: a 14sp line at `alpha="0.7"` reads as descriptive text, so nobody found the picker in it. The row greys out (`DISABLED_ALPHA`, and `setEnabled(false)` on the value is what actually blocks the tap) while the notification switch is off, driven from `syncNotificationTimeRow()` — called from `setNotificationSwitchChecked()`, the switch listener and the permission result. The time views are found in `onCreate` *before* the switch, since `syncNotificationSwitch()` reaches them. The picker follows the system 12/24-hour setting (`DateFormat.is24HourFormat`) and the summary is formatted with `DateFormat.getTimeFormat`, so it reads as `8:00 AM`, `08:00` or `上午8:00` depending on the reader. It is themed with `ThemeOverlay.VerseApp.TimePicker`, which puts a real `colorPrimary` back for the same reason the red-letter dialog forces its button colour: the app repurposes `colorPrimary` to match `colorSurface`, and the picker paints the clock hand, the selected number and the OK/Cancel text with it. A picker showing across an Activity recreation is rebuilt by the `FragmentManager` without its listener, so `onCreate` re-attaches one to any survivor found by tag.
...
- **`SharedPreferences("settings")`** — keys: `"theme_mode"` (string: `"light"`/`"dark"`/`"system"`), `"translation"` (string: `"kjv"`/`"asv"`/`"bsb"`/`"rvr1909"`/`"cuvs"` — written on first read from the device language, and moved when the system language changes; see "Translation selection"), `"translation_language"` (string: the device language the stored translation was chosen under), `"translation_manual"` (boolean: set once the reader picks a translation in Settings, which stops the language sync), `"show_translation_info"` (boolean), `"widget_verse_ref"` + `"widget_verse_day"` (see Home-screen widget), `"browse_last_book"` + `"browse_last_chapter"` (ints, default -1 — see `BrowseActivity`), `"daily_verse_notification"` (boolean, default false) + `"daily_verse_notification_time"` (int: minutes since midnight, default 480 = 08:00 — both see Daily verse notification). The older `"theme"` boolean key is legacy/dead (still read in `Tools.java` but not written or used by any active code path).

  A `"search_history"` key (the last 20 queries, JSON array) used to be written here by `MainActivity.saveSearchHistory()`. It was never read back — no suggestions or autocomplete UI was ever built on it, so it was write-only from the day it landed — and both the method and the key are gone. `MainActivity.onCreate` carries a `contains()`-guarded `remove("search_history")` to clear the orphan from installs that already accumulated one; that line is disposable once existing installs have all launched once. Don't reintroduce the key without the UI that consumes it: it is a record of what the reader typed, and (see below) it would back up to their next device.

### Backup and restore
Auto Backup is on (`android:allowBackup="true"`) and configured by two rules files, both wired onto `<application>`: `res/xml/data_extraction_rules.xml` for Android 12+ (`<cloud-backup>` and `<device-transfer>`) and `res/xml/backup_rules.xml` for Android 8–11, which is why there are two — minSdk is 27, so both paths are live. Keep them in step.

Both are **exclude-only**, so everything else is backed up by default: that is what carries the bookmarks Room DB and all of `SharedPreferences("settings")` onto a new device. The single exclusion is `review_prefs.xml` — `PlayStoreReviewPrompt`'s counters are install-local, and restoring `review_requested=true` would mean a reader who changes phones is never offered the in-app review again. Note SharedPreferences rules are **file**-granular: an individual key cannot be excluded, which is why `"daily_verse_notification"` still restores (as `true`, with `POST_NOTIFICATIONS` not restored alongside it — `VerseNotifier.notifyToday`'s `hasPermission` guard makes that silent, and `SettingsActivity.syncNotificationSwitch()` corrects the switch when the reader next opens Settings). Excluding any single `"settings"` key means moving it to its own prefs file first.

**Do not convert the database domain to an `<include>` rule.** Room's default journal mode is WAL, so the DB on disk is `bookmarks-database` plus `bookmarks-database-wal` (and `-shm`), and an `<include>` path matches that one exact file name — it would silently drop the `-wal`, where recent bookmarks live until SQLite checkpoints them. The default include covers the whole directory; both XML files carry this warning too.

Verified end to end on a device (Android 16) over `LocalTransport`, which is the way to test this — the Google transport won't back up on demand:
```sh
adb shell bmgr transport com.android.localtransport/.LocalTransport   # remember the old one, put it back after
adb shell bmgr backupnow com.verse.of.the.day
adb uninstall com.verse.of.the.day && adb install app-debug.apk       # restore fires automatically on install
adb shell run-as com.verse.of.the.day ls shared_prefs databases       # review_prefs.xml must be absent
```
Take a `run-as ... tar cf -` snapshot of `databases`/`shared_prefs` off the device first — the uninstall wipes real bookmarks, and if backup misbehaves that tar is the only way back.
